...
function bp_wa_oauth_trigger_register(redirectOnComplete) {

    const loginUri = '/wp-json/bp-wa-oauth/v1/oauth/register';
...
window.addEventListener('click', function (event) {
    ...
    var registerTriggerClass = 'bp-wa-oauth-register';
    ...
    if (event.target.className.indexOf(registerTriggerClass) > -1 || event.target.parentElement.className.indexOf(registerTriggerClass) > -1) {
        if (typeof event.target.dataset.bpWaOauthRedirect !== 'undefined') {
            bp_wa_oauth_trigger_register(event.target.dataset.bpWaOauthRedirect);
        } else {
            bp_wa_oauth_trigger_register();
        }
    }
});
